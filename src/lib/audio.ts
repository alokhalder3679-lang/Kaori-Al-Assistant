...
export class MyraaAudioSession {
  ...
  private onSessionMemorySync?: (keyPoints: string[]) => void;
  ...
  constructor(handlers: {
    ...
    onSessionMemorySync?: (keyPoints: string[]) => void;
  }) {
    ...
    this.onSessionMemorySync = handlers.onSessionMemorySync;
  }
  ...
  /**
   * Sends a manual session reset signal to the server.
   */
  public clearSession() {
  ...
  /**
   * Pushes a raw text message directly to the live WebSocket server for typing fallback.
   */
  public sendTextMessage(textValue: string) {
    if (this.ws && this.ws.readyState === WebSocket.OPEN && this.currentState !== "disconnected") {
      this.ws.send(JSON.stringify({ type: "text", text: textValue }));
    }
  }
    ...
    try {
      ...
      this.ws.onopen = async () => {
        ...
        try {
          ...
          try {
            ...
            console.warn("Microphone access declined or unavailable. Continuing in silent-input mode:", micErr);
            this.onError("Microphone permission denied or unavailable. Connected in Text/Keyboard mode! Use the input bar below to type to Myraa.");
          }
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          // Handle active session memory key points synchronization
          if (data.type === "session_memory_sync" && data.keyPoints) {
            if (this.onSessionMemorySync) {
              this.onSessionMemorySync(data.keyPoints);
            }
          }
