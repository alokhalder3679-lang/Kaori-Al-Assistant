...
export async function extractSessionKeyPoints(
  apiKey: string,
  dialogueHistory: { role: string; text: string }[]
): Promise<string[] | null> {
  if (dialogueHistory.length < 2) {
  ...
  try {
    ...
    const prompt = `You are Myraa's active conversation short-term memory analyzer. Your task is to analyze the history of the current ongoing conversation session/call, and extract a list of 3 to 10 key bullet-point-style facts, topics, context points, decisions, or preferences that have been established *during this specific conversation session*.

### OBJECTIVE:
Compile an accurate, concise, factual list summarizing what has occurred in this active call so far.
Focus on:
- Crucial facts mentioned by the user or discussed (e.g. what project they are currently looking at, questions they have asked).
- Decisions made or tasks Myraa has performed (e.g., opened a website, searched for something).
- Fleeting preferences or goals expressed during this conversation.
Avoid cataloging empty chit-chat, greetings ('hello', 'how are you'), or trivial acknowledgements.

### CURRENT CONVERSATION RECORD:
${dialogueContext}

### RULES:
- Output a clean list of declarative third-person summaries (e.g., 'The user is asking about React rendering optimization.', 'Myraa opened a YouTube search for lo-fi beats.').
- Do not include conversational filler, quotes, or timestamps.`;
        ...
        responseSchema: {
          ...
          properties: {
            keyPoints: {
              ...
              description: "List of concise key points or facts established during this session so far."
            }
          },
          required: ["keyPoints"]
        }
    ...
    const resultText = response.text?.trim() || "{}";
    const resultObj = JSON.parse(resultText);
    return resultObj.keyPoints || [];

  } catch (error) {
    console.error("[Session Memory] Extraction failure:", error);
    return null;
  }
}
